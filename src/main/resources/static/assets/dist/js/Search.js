...
$(document).on('keyup', '#input-table-search', function () { // when the input for the flight table is changed, modify results
    var strInputVal = $(this).val();

    ChangeTable(strInputVal);
});
...
$(document).on('click', '#BookFlight', function () {
    ...
    $.ajax({
        url: `/Search/BookFlight/${FlightID}`,
        type: 'GET',
        data: {},
        success: function(data) {
            if (data) {
                window.location = "/UserFlights";
            }
            else {
...
var TableRowButton = function (FlightID) {
    return `<td style="width: auto !important;"><button class="btn btn-primary my-2" id="BookFlight" value="${FlightID}">Book Flight</button></td>`;
}
